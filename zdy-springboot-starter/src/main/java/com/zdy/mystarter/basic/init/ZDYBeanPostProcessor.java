...
public class ZDYBeanPostProcessor implements BeanPostProcessor{
    ...
    public Object postProcessBeforeInitialization(Object bean, String beanName) throws BeansException {
        ...
        if(beanName.contains("hello")){
            logger.info("实例化完成之后调用：BeanPostProcessor.postProcessBeforeInitialization--->"+beanName+"=>"+bean);
        }
        //不代理直接返回
        if(!isProxy(bean)){
            return bean;
    ...
    public Object postProcessAfterInitialization(Object bean, String beanName) throws BeansException {
        // TODO Auto-generated method stub
        if(beanName.contains("hello")){
            logger.info("全部是实例化完成以后调用该方法：BeanPostProcessor.postProcessAfterInitialization-->"+beanName+"=>"+bean);
        }
        return bean;
    }
